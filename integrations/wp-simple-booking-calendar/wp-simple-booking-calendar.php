 ...
 * @since wp-simple-booking-calendar    2.0.10
 */
class Zoho_Flow_WP_Simple_Booking_Calendar extends Zoho_Flow_Service{
    ...
    public function list_calendars( $request ){
        ...
        if( ! class_exists( 'WPSBC_Object_DB_Calendars' ) ) {
            require_once( ABSPATH . 'wp-content/plugins/wp-simple-booking-calendar/includes/base/calendar/class-object-db-calendars.php' );
        }
        $calendar_obj = new WPSBC_Object_DB_Calendars();
    ...
    public function list_legend_items( $request ){
        ...
        if( ! class_exists( 'WPSBC_Object_DB_Legend_Items' ) ) {
            require_once( ABSPATH . 'wp-content/plugins/wp-simple-booking-calendar/includes/base/legend/class-object-db-legend-items.php' );
        }
        $legend_obj = new WPSBC_Object_DB_Legend_Items();
    ...
    public function fetch_event( $request ){
        ...
        if( isset( $fetch_field ) && isset( $fetch_value ) && in_array( $fetch_field, $allowed_fetch_feilds) ){
            ...
            elseif ( 'event_date' === $fetch_field && strtotime( $fetch_value ) ){
                if( ! class_exists( 'WPSBC_Object_DB_Events' ) ) {
                    require_once( ABSPATH . 'wp-content/plugins/wp-simple-booking-calendar/includes/base/event/class-object-db-events.php' );
                }
                $event_obj = new WPSBC_Object_DB_Events();
    ...
    public function create_webhook( $request ){
        ...
        if( ( isset( $entry->name ) ) && ( isset( $entry->url ) ) && ( isset( $entry->event ) ) && ( in_array( $entry->event, self::$supported_events ) ) && ( preg_match( "/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $entry->url ) ) ){
            ...
            $post_name = "WP Simple Booking Calendar ";
    ...
    public function get_system_info(){
        ...
        $plugin_dir = ABSPATH . 'wp-content/plugins/wp-simple-booking-calendar/wp-simple-booking-calendar.php';
        if(file_exists( $plugin_dir ) ){
            $plugin_data = get_plugin_data( $plugin_dir );
            $system_info['wp_simple_booking_calendar'] = @$plugin_data['Version'];
        }
