 ...
 * @since kali-forms    2.3.47
 */
class Zoho_Flow_Kali_Forms extends Zoho_Flow_Service{
    ...
    public function list_form_fields( $request ){
        ...
        if( $this->is_valid_form( $form_id ) ){
            $form_fields = get_post_meta( $form_id, 'kaliforms_field_components' );
            $fields_return_list = array();
            foreach ($form_fields as $index => $data){
    ...
    private function is_valid_form( $form_id ){
        if( isset( $form_id ) ){
            if( "kaliforms_forms" === get_post_type( $form_id ) ){
                return true;
            }
            return false;
        }
     ...
     */
    public function payload_form_entry_submitted( $form_args ){
        $post_id = $form_args['data']['formId'];
        $args = array(
    ...
    public function get_system_info(){
        ...
        $plugin_dir = ABSPATH . 'wp-content/plugins/kali-forms/kali-forms.php';
        if(file_exists( $plugin_dir ) ){
            $plugin_data = get_plugin_data( $plugin_dir );
            $system_info['kali_forms'] = $plugin_data['Version'];
        }
